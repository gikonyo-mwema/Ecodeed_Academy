...
class BaseSocialAuthView(APIView):
    ...
    def get_or_create_user(self, email, first_name, last_name, social_id_field, social_id, profile_picture=''):
        """
...
class TwitterLoginView(APIView):
    ...
    def get(self, request):
        ...
        client_id = getattr(settings, 'TWITTER_CLIENT_ID', None) or \
                    settings.SOCIALACCOUNT_PROVIDERS.get('twitter', {}).get('APP', {}).get('client_id')
...
class TwitterCallbackView(BaseSocialAuthView):
    ...
    def post(self, request):
        ...
        client_id = getattr(settings, 'TWITTER_CLIENT_ID', None) or \
                    settings.SOCIALACCOUNT_PROVIDERS.get('twitter', {}).get('APP', {}).get('client_id')
        client_secret = getattr(settings, 'TWITTER_CLIENT_SECRET', None) or \
                        settings.SOCIALACCOUNT_PROVIDERS.get('twitter', {}).get('APP', {}).get('secret')
